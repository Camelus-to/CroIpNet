...
def process_dynamic_connectivity_files(input_dir, dynamic_matrix_dir, output_dir, df):
    ...
    for filename in os.listdir(dynamic_matrix_dir):
        if filename.endswith('.mat'):
            ...
            signals = loadmat(signals_path)['ROISignals'][:100, 228:428].T  # The format of the signal should be region*signal length
            if np.all(signals == 0):
                print(f"Warning: Time series data for {filename} contains all zeros")
    ...
    output_file = os.path.join(output_dir, 'connectivity_data_abide_cc200_qu.npy')
