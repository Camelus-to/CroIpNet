...
# Generate new training samples by mixing data and labels to enhance the model's generalization and robustness.
def continus_mixup_data(*xs, y=None, alpha=1.0, device='cpu'):
    '''Returns mixed inputs, pairs of targets, and lambda'''
    if alpha > 0:
        # Generate a random number lam from the Beta distribution. Lam is the coefficient controlling the mixing ratio.
        lam = np.random.beta(alpha, alpha)
    else:
        lam = 1
    batch_size = y.size()[0]
    # Generate a randomly permuted index, which is used to shuffle the samples in the batch for mixing
    index = torch.randperm(batch_size).to(device)
    # For each tensor in the input data xs, linearly combine the current batch samples and the shuffled samples according to lam and (1-lam) to generate new input data new_xs.
