...
class TopologicalVector(BaseEstimator, TransformerMixin):
    ...
    def chebyshev_distance_matrix(self, X):
        """
        Calculate the Chebyshev distance matrix for a set of points.

        Parameters:
            X (numpy array): An array of shape (n_samples, n_features) representing n_samples points in an n_features-dimensional space.

        Returns:
            dist_matrix (numpy array): A Chebyshev distance matrix of shape (n_samples, n_samples).
        """
        n_samples = X.shape[0]
        ...
        for i in range(n_samples):
            for j in range(n_samples):
                dist_matrix[i, j] = np.max(np.abs(X[i] - X[j]))
    ...
    def transform(self, X):
        ...
        for i in range(num_diag):
            ...
            distances = self.chebyshev_distance_matrix(diagram)
