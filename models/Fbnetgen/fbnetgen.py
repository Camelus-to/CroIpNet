...
class FBNETGEN(nn.Module):
    ...
    def forward(self, final_pearson, signals, pseudo, ages, genders, site, semantic_similarity_matrix):
        x = self.extract(signals)
        x = F.softmax(x, dim=-1)
        m = self.emb2graph(x)
        m = m[:, :, :, 0]

        return [self.predictor(m, final_pearson),  m], m, 0, 0
