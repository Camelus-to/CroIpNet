...
class Utils():
    # This method is mainly the mixup data augmentation method. y_a is the original label, y_b is the shuffled label
    def mixup_data(x, nodes, pcorr, y, alpha=1.0, device='cuda'):
        ...
        index = torch.randperm(batch_size).to(
            device)  # Randomly shuffled vector [15,  1,  4,  3, 12,  5, 10, 13,  9, 14,  6, 11,  0,  2,  7,  8]

        mixed_nodes = lam * nodes + (1 - lam) * nodes[index, :]
        mixed_x = lam * x + (1 - lam) * x[index, :]
        mix_pcorr = lam * pcorr + (1 - lam) * pcorr[index, :]
        # mix_te = lam * te + (1 - lam) * te[index, :]
        y_a, y_b = y, y[index]
        return mixed_x, mixed_nodes, mix_pcorr, y_a, y_b, lam
