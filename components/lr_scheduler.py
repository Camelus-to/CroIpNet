...
class LRScheduler:
    def __init__(self, cfg: DictConfig,  optimizer_cfg: DictConfig):
        self.lr_config = optimizer_cfg.lr_scheduler # optimizer_cfg这个是Adam得整个参数； 然后经过这个定义之后self.lr_config是lr_scheduler里面的优化策略
    ...
    def update(self, optimizer: torch.optim.Optimizer, step: int):
        lr_config = self.lr_config  # 其实就是是lr_scheduler里面的优化策略
